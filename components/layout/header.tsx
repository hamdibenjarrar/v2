  ...
  const mobileNavItems = (
    <>
      <div className="flex flex-col gap-2">
        <h3 className="font-bold">{t('whoWeAre')}</h3>
      ...
      <div className="flex flex-col gap-2">
        <h3 className="font-bold">{t('ourImpact')}</h3>
        {ourImpactLinks.map(link => <Link key={link.href} href={link.href} className="text-muted-foreground" onClick={() => setIsMobileMenuOpen(false)}>{link.label}</Link>)}
      </div>
       <div className="flex flex-col gap-2">
        <h3 className="font-bold">{t('joinMovement')}</h3>
        {joinMovementLinks.map(link => <Link key={link.href} href={link.href} className="text-muted-foreground" onClick={() => setIsMobileMenuOpen(false)}>{link.label}</Link>)}
      </div>
       <div className="flex flex-col gap-2">
        <h3 className="font-bold">{t('stories')}</h3>
        {storiesLinks.map(link => <Link key={link.href} href={link.href} className="text-muted-foreground" onClick={() => setIsMobileMenuOpen(false)}>{link.label}</Link>)}
      </div>
    </>
  );
        ...
        <div className="flex items-center justify-end space-x-2 rtl:space-x-reverse">
          ...
          <div className="md:hidden">
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              ...
              <SheetContent side="right" className="w-full max-w-sm overflow-y-auto bg-white/80 backdrop-blur-md">
